...
| Stateflow 构造 | 它表达的行为 | 当前导入结论 |
|---|---|---|
| 普通/层次状态 | 一个活动状态，或状态内部继续分层 | 支持单活动路径 |
| `entry` / `during` / `exit` | 进入、每拍、离开时执行动作 | 仅简单 `entry`/`exit` 赋值稳定支持 |
| event / message | 由宿主在运行时投递的触发 | 暂不映射 |
| junction / history | 路径分支，或恢复上次子状态 | 暂不映射 |
| condition action | 试探路径时执行的动作 | 暂不映射 |
| AND 状态 | 多个区域同时活动 | 暂不映射 |
| graphical/MATLAB function | 可读写数据的函数代码 | 暂不映射 |
| `-1` 尺寸 | 从端口或配置继承的编译尺寸 | 必须读取编译上下文 |
...
history 的直观例子是：`Door` 上次离开时停在 `Opening`，再次进入 `Door` 时不从固定初始子状态开始，而是回到 `Opening`。如果只把它改写成 `[*] -> Closed`，恢复位置就被删除，后续轨迹会改变；因此当前报告为不支持，而不是自动选择一个看似合理的子状态。
